...
### 2. 로컬 HTML 작업 (작업용.html)
- `작업용.html`이 메인 작업 파일입니다. 여기서 자유롭게 디자인합니다.
  - 메인 비주얼 (로고, 배경, 쿠폰, 타이틀)
  - 카카오 톡채널 친구추가 버튼
  - 방송 혜택 (양말/바디백/반팔티 증정)
...
### 3. 가격 데이터 업데이트
- 엑셀(`final/new.xlsx`) 기준으로 89개 상품의 가격을 `작업용.html`에 일괄 반영
  - 정상가 (D열 택가)
  - 톡딜가 (E열)
  - 쿠폰가 (G열 톡딜 혜택가)
...
### 8. 최종 HTML 생성 (kakao_final_YYMMDD.html)
- 카카오 편집기 호환 형태로 생성 (이미지 기반)
- `작업용.html`에서 캡처한 이미지들로 구성
- 파일명 규칙: `kakao_final_260804.html` (날짜 기반)
- 구조:
```
&lt;!-- 상단 이미지 --&gt;
...
├── kakao_final_260804.html ← ★ 최종 산출물 (카카오 편집기 붙여넣기용, 이미지 기반)
├── guide.md          ← 이 파일
├── .env              ← API 키
...
4. **가격 변경 시**: `final/new.xlsx` 수정 → `작업용.html` 업데이트 → 카드 이미지 재캡처 필요
5. **컬러칩 변경 시**: `작업용.html`에서 해당 컬러 hex 값 직접 수정 → 카드 이미지 재캡처
